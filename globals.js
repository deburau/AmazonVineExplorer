...
async function waitForHtmlElmement(selector, cb, altDocument = document) {
    ...
    const _observer = new MutationObserver(_mutations => { // Renamed mutations to _mutations
        if (altDocument.querySelector(selector)) {
...
function generateSessionID() {
    return 'aaaa-aaaaa-AVE-SESSION-aaaaaaa-aaaaaaaa'.replace(/[a]/g, ( _c ) => { return Math.round(Math.random() * 36).toString(36) });
}
